...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/RheixParams"
else
    PARAMS_DIR="$HOME/.rheix-params"
fi
...
DOWNLOAD_URL="https://github.com/rheixverse/rheix/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with RHX_DISABLE_SOMETHING=1
RHX_DISABLE_WGET="${RHX_DISABLE_WGET:-}"
#RHX_DISABLE_IPFS="${RHX_DISABLE_IPFS:-}"
RHX_DISABLE_CURL="${RHX_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$RHX_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$RHX_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$RHX_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Rheix parameters!
...
This script will fetch the Rheix SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Rheix SNARK parameters. Note that it is
