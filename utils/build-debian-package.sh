...
PACKAGE_VERSION=$($SRC_PATH/src/rheixd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/rheixd $DEB_BIN
cp $SRC_PATH/src/rheix-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/rheix-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/rheixd.1 $DEB_MAN
cp $SRC_DOC/man/rheix-cli.1 $DEB_MAN
cp $SRC_DOC/man/rheix-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/rheixd.bash-completion $DEB_CMP/rheixd
cp $SRC_PATH/contrib/rheix-cli.bash-completion $DEB_CMP/rheix-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/rheixd.1
gzip --best -n $DEB_MAN/rheix-cli.1
gzip --best -n $DEB_MAN/rheix-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/rheixd $DEB_BIN/rheix-cli
