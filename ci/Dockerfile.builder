...
# rheix_hash
RUN git clone https://github.com/rheixverse/rheix_hash
RUN cd rheix_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} rheix
RUN useradd -u ${USER_ID} -g rheix -s /bin/bash -m -d /rheix rheix
...
RUN mkdir /rheix-src && \
  ...
  chown $USER_ID:$GROUP_ID /rheix-src && \
