...
const char * const BITCOIN_CONF_FILENAME = "rheix.conf";
const char * const BITCOIN_PID_FILENAME = "rheixd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "rheixd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RheixCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RheixCore
    // Mac: ~/Library/Application Support/RheixCore
    // Unix: ~/.rheixcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RheixCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RheixCore";
#else
    // Unix
    return pathRet / ".rheixcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty rheix.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Rheix Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Rheix Core developers";
    }
