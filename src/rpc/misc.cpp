...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"rheix\" activates all Rheix-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "rheix")
            + HelpExampleRpc("debug", "rheix+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given rheix address.\n"
            ...
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The rheix address to validate\n"
            "\nResult:\n"
            "{\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The rheix address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "rheix-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nrheix-cli has the option -getinfo to collect and format these in the old format."
    );
}
